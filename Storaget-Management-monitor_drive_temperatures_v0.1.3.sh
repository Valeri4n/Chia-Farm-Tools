...
#  mnt=($(mount | grep /dev/sd | awk '{print $3}' | awk -F '/' '{print $NF}'))
  arr_len=${#drv[@]}
#  for i in /mnt/sd*; do
  imax=$(($arr_len - 1))
  for (( i=0; i<=$imax; i++ )); do
    mnt_prefix=`cut -d/ -f2 <<< ${path[$i]}`
    if (([ -z $mount_point ] && ([ "$mnt_prefix" = mnt ] || [ "$mnt_prefix" = home ])) || [ "$mnt_prefix" = "$mount_point" ]); then # && [ $((i)) -lt 5 ]; then
      ...
      if [ $((SMART)) -eq 1 ]; then
        ...
        if [ -z $temp ]; then
          temp=`smartctl -A ${drv[$i]} 2>/dev/null | grep "194 Temperature_Celsius" | awk '{print $4}' | sed 's/^0//'`
  #        temp=`smartctl -l devstat /dev/${i:5} 2>/dev/null | grep 'Current Temperature' | awk $'{print $4}'`
        fi
      fi
...
#          printf "\nIssue with drives: $INVALIDS\n"
      ...
      if [ $((temp)) -ge $((HITEMP)) ] || [ -z $HITEMP ]; then
